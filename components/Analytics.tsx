...
const Analytics: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-slate-800/50 p-8 rounded-2xl border border-slate-700">
        <h3 className="text-2xl font-bold mb-6">File Operations Risk Analysis (Top 15)</h3>
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={fileActivityData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
              <XAxis dataKey="name" angle={-45} textAnchor="end" height={80} tick={{ fontSize: 11 }} stroke="#94a3b8" />
              <YAxis stroke="#94a3b8" />
              <Tooltip contentStyle={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 8 }} />
              <Legend />
              <Bar dataKey="fileOps" fill="#f97316" name="File Ops (Delete+Copy)" />
              <Bar dataKey="fileAccess" fill="#06b6d4" name="File Access" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
