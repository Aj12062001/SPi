  ...
  return (
    <div className="activity-visualization space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Database Activity */}
        <div className="bg-slate-900 p-6 rounded-xl border border-slate-700">
          <h4 className="text-lg font-bold text-white mb-4">🗄️ Database Activity</h4>
          <div className="space-y-3">
            <div className="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg">
              <span className="text-slate-400">Primary Database</span>
              <span className="text-sm font-bold text-indigo-300">{employeeData.primary_database || 'N/A'}</span>
            </div>
            <div className="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg">
              <span className="text-slate-400">DB Session Duration</span>
              <span className="text-xl font-bold text-cyan-400">{employeeData.database_session_duration || 0} min</span>
            </div>
            <div className="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg">
              <span className="text-slate-400">DB Query Count</span>
              <span className="text-xl font-bold text-blue-400">{employeeData.database_query_count || 0}</span>
            </div>
            <div className="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg">
              <span className="text-slate-400">DB Read / Write</span>
              <span className="text-sm font-bold text-slate-200">{employeeData.database_read_ops || 0} / {employeeData.database_write_ops || 0}</span>
            </div>
          </div>
        </div>
      </div>
