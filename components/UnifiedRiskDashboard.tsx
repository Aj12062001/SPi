...
`;
  ...
  const simulateDetection = () => {
    ...
    // Randomly select an employee with high risk
    const highRiskEmployees = employeeData.filter(emp => {
      const score = riskAssessments.get(emp.user)?.overallRiskScore || emp.risk_score || 0;
      return score >= 60;
    });
    
    const employee = highRiskEmployees.length > 0 
      ? highRiskEmployees[Math.floor(Math.random() * highRiskEmployees.length)]
      : employeeData[Math.floor(Math.random() * employeeData.length)];
    
    setDetectedEmployee(employee);
    
    // Build simulated activities from job operations
    const activities: ActivityLog[] = [];
    if (employee.file_operations_detail) {
      try {
        const operations = JSON.parse(employee.file_operations_detail);
        if (Array.isArray(operations)) {
          operations.slice(0, 5).forEach((op: any, idx: number) => {
            activities.push({
              id: `sim_${Date.now()}_${idx}`,
              userId: employee.user,
              timestamp: new Date(Date.now() - Math.random() * 3600000).toISOString(),
              activityType: 'file_accessed',
              severity: (employee.risk_score || 0) >= 80 ? 'critical' : 'high',
              isAnomalous: true,
              details: {
                fileName: op?.file_name || 'File',
                operation: op?.operation || 'accessed',
                isSensitive: Boolean(op?.is_sensitive)
              },
              duration: 120
            });
  ...
  const sendRiskAssessmentEmail = async (entries: any[], isAutomatic = false) => {
    ...
    try {
      ...
      if (!response.ok) {
        const detailMessage = getEmailErrorMessage(payload, response.statusText || 'Email sending failed');
        setEmailStatus(`❌ Email failed: ${detailMessage}`.trim());
        return;
      }
    ...
    <div className="space-y-8 pb-8">
      ...
      <div className="bg-slate-900/80 backdrop-blur-md p-6 rounded-2xl border border-slate-700">
        ...
        <div className="flex flex-col md:flex-row gap-3">
          ...
          />
          <button
            onClick={() => sendRiskAssessmentEmail(elevatedRiskEntries)}
            disabled={isSendingEmail || !alertSenderEmail.trim() || !alertRecipientEmail.trim() || elevatedRiskEntries.length === 0}
            className="flex items-center justify-center gap-2 px-5 py-3 bg-green-600 hover:bg-green-700 disabled:bg-slate-700 disabled:cursor-not-allowed text-white rounded-lg font-semibold"
          >
            <Send size={18} />
      ...
      {/* Simulate Detection Section */}
      <div className="bg-slate-900/80 backdrop-blur-md p-6 rounded-2xl border border-slate-700">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-xl font-bold text-white">🎯 Simulate Detection</h3>
            <p className="text-slate-400 text-sm mt-1">Simulate detecting an employee and view their activities</p>
          </div>
          <button
            onClick={simulateDetection}
            className="flex items-center justify-center gap-2 px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-semibold transition-all shadow-lg hover:shadow-purple-500/20"
          >
            🔍 Simulate Detection
          </button>
        </div>
        ...
        {detectedEmployee && (
          <div className="mt-6 p-4 bg-slate-800/50 rounded-xl border border-slate-700">
            <p className="text-sm text-slate-300 mb-2">
              <span className="font-semibold">Detected Employee:</span> {detectedEmployee.employee_name || detectedEmployee.user}
            </p>
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {simulatedActivities.length > 0 ? (
                simulatedActivities.map((activity, idx) => (
                  <div key={idx} className="text-xs text-slate-400 p-2 bg-slate-700/30 rounded border border-slate-700">
                    <span className="font-mono">{new Date(activity.timestamp).toLocaleTimeString()}</span>
                    {' - '}
                    <span className="text-blue-300">{activity.details?.operation || 'accessed'}</span>
                    {' - '}
                    <span className="text-slate-300">{activity.details?.fileName || 'File'}</span>
                    {activity.details?.isSensitive && (
                      <span className="ml-2 text-red-400">⚠️ SENSITIVE</span>
                    )}
                  </div>
                ))
              ) : (
