...
const Introduction: React.FC = () => {
  ...
  const storedCctvResults = (() => {
    const raw = localStorage.getItem('cctvDetectionResults');
  ...
  const anomalies = useMemo(() => {
    ...
    return activityLogs.filter((a) => a.isAnomalous).length;
  }, [activityLogs]);

  const contaminationRate = useMemo(() => {
    if (activityLogs.length === 0) return 0;
    return Math.min(100, (anomalies / activityLogs.length) * 100);
  }, [activityLogs, anomalies]);

  const physicalBreaches = 0;

  const forestPrecision = 88.2;
  ...
  return (
    <div className="space-y-12">
      <section className="relative overflow-hidden rounded-3xl border border-indigo-500/30 bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 p-10 shadow-2xl w-full min-h-[360px]">
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-5 gap-10 items-center">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 xl:grid-cols-4 gap-4 items-stretch">
              {metrics.map((metric) => (
                <div
                  key={metric.label}
      ...
      <div className="grid grid-cols-1 gap-10">
        <div className="space-y-8">
          <section className="bg-slate-800/50 p-8 rounded-3xl border border-slate-700 w-full min-h-[360px]">
          ...
          <section className="bg-slate-800/50 p-8 rounded-3xl border border-slate-700 w-full min-h-[360px]">
            <h3 className="text-2xl font-bold mb-6 text-indigo-300 flex items-center gap-2"><span className="text-indigo-500">◆</span> Execution Steps & Technical Requirements</h3>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                <h4 className="text-lg font-semibold text-white mb-4">Execution Steps</h4>
              ...
              <div className="bg-indigo-900/20 p-6 rounded-2xl border border-indigo-500/30 shadow-lg shadow-indigo-500/10 h-full">
